...
function Editrent() {
  ...
  useEffect(() => {
    fetch(`http://localhost:9000/edit/rent/${id}`)
      .then(response => response.json())
      .then(data => {
        console.log('Fetched house:', data);
        if (data.success) {
          setHouse(data.rent); // Update state with fetched rent data
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:9000/update/rent/${id}`, {
